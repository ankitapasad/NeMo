...
# Adapted from examples/asr/transcribe_speech.py
...
from nemo.collections.asr.parts.utils.transcribe_utils import (
    ...
    setup_model,
    write_transcription,
    generate_manifest_with_context,
    generate_final_manifest_for_eval,
    load_json
)
...
# Arguments
  ...
  dataset_manifest_og: path to unchunked dataset JSON manifest file (in NeMo format)
  ...
  decode_with_context: Bool to use previous decoded output as context to the current chunk
  ...
  output_filename_combined: Output filename where the combined transcriptions will be written
...
class TranscriptionConfig:
    ...
    dataset_manifest_og: Optional[str] = None  # Path to unchunked dataset's JSON manifest
    ...
    output_filename_combined: Optional[str] = None
    batch_size: int = 32
    num_workers: int = 0
    ...
    # set to True to use output of preceeding chunk as context
    decode_with_context: bool = False
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    # create a directory to save updated manifest files, copy the 1st chunk and update cfg.dataset_manifest
    manifest_dir = os.path.join(os.path.dirname(cfg.dataset_manifest), '..', f'decoded_{os.path.basename(cfg.model_path)}')
    os.makedirs(manifest_dir, exist_ok=True)
    os.makedirs(os.path.dirname(cfg.output_filename), exist_ok=True)
    first_manifest_fn = os.path.join(os.path.dirname(cfg.dataset_manifest), 'manifest_0.json')
    first_manifest_cp_fn = os.path.join(manifest_dir, 'manifest_0.json')
    shutil.copy(first_manifest_fn, first_manifest_cp_fn)
    num_manifests = len(glob(os.path.join(os.path.dirname(cfg.dataset_manifest), 'manifest_*.json')))
    if cfg.decode_with_context:
        cfg.dataset_manifest = first_manifest_cp_fn

    # Transcribe audio
    # Process manifest chunks sequentially
    prev_idx = 0

    for manifest_idx in range(num_manifests):
        cfg.dataset_manifest = cfg.dataset_manifest.replace(f'_{prev_idx}.json', f'_{manifest_idx}.json')
        cfg.output_filename = cfg.output_filename.replace(f'_{prev_idx}.json', f'_{manifest_idx}.json')
        ...
        num_files = len(load_json(filepaths))
        print(num_files)
        print(len(transcriptions))
        print(cfg.dataset_manifest)
        if num_files != len(transcriptions):
        ...
        # manage manifests
        if manifest_idx < num_manifests-1 and cfg.decode_with_context:
            generate_manifest_with_context(manifest_idx, cfg.output_filename, cfg.dataset_manifest, cfg.context_len)
        prev_idx = manifest_idx
        # import pdb; pdb.set_trace()
        
    generate_final_manifest_for_eval(
        os.path.dirname(cfg.output_filename),
        cfg.dataset_manifest_og,
        cfg.output_filename_combined,
        num_manifests,
        )
    if cfg.calculate_wer:
        output_manifest_w_wer, total_res, _ = cal_write_wer(
            pred_manifest=cfg.output_filename_combined,
