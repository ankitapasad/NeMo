...
class DuplexS2SSpeechDecoderModel(LightningModule, HFHubMixin):
    ...
    def forward(
        ...
        if self.cfg.get("use_separate_asr_head", False):
            ...
            asr_logits = self.asr_head(asr_in)
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        if batch["audio_data"] is not None:
            ...
            with loss_parallel():
                ...
                with torch.no_grad():
                    ...
                    target_tokens = inputs["text_labels"]
                ...
                ans = {
                    ...
                    "asr_loss": asr_loss if self.cfg.get("use_separate_asr_head", False) else text_loss,
                    "batch": B,
                    "length": T,
        ...
        if batch["text_data"] is not None:
            ...
            text_logits = self.lm_head(text_out['last_hidden_state'])
    ...
    def init_streaming_inference(
        ...
        """
        Initialize streaming inference with audio buffer and window parameters.
        
        Usage Example:
            >>> # Load audio
            >>> audio, sr = torchaudio.load("test.wav")
            >>> audio = audio.to(device).unsqueeze(0)  # [1, T_samples]
            >>> 
            >>> # Initialize streaming
            >>> model.init_streaming_inference(audio, window_size=70, stride=1)
            >>> 
            >>> # Get embeddings frame by frame (simulating real-time processing)
            >>> for t in range(model.streaming_total_frames):
            >>>     input_embed = model.get_input_embeds(t)  # [1, H]
            >>>     # Now you can feed input_embed to LLM for frame t
            >>>     # llm_output = model.llm(input_embed)
            >>> 
            >>> # Or get embeddings for specific frames
            >>> emb_frame_0 = model.get_input_embeds(0)    # First frame
            >>> emb_frame_50 = model.get_input_embeds(50)  # Frame 50
            >>> emb_frame_100 = model.get_input_embeds(100) # Frame 100
        
        Args:
            input_signal: Full audio waveform [B, T_samples]
            window_size: Number of frames to include in each encoder forward pass (context window)
            stride: Number of frames to advance each step (typically 1 for streaming)
        
        Note:
            - window_size determines the receptive field (e.g., 70 frames = 70 * 0.08s = 5.6s)
            - stride=1 means we process one new frame at a time (true streaming)
            - The audio is stored and we extract windows on-the-fly in get_input_embeds()
        """
        self.streaming_audio = input_signal  # [B, T_samples]
        self.streaming_window_size = window_size
        self.streaming_stride = stride
        self.streaming_frame_length = 0.08  # 80ms per frame
        self.streaming_samples_per_frame = int(self.streaming_frame_length * self.source_sample_rate)
        
        # Calculate total number of frames in the audio
        B, T_samples = input_signal.shape
        self.streaming_total_frames = T_samples // self.streaming_samples_per_frame
        
        logging.info(
            f"Initialized streaming inference: "
            f"window_size={window_size} frames ({window_size * 0.08:.2f}s), "
            f"stride={stride}, "
            f"total_frames={self.streaming_total_frames}"
        )

    @torch.no_grad()
    def get_input_embeds(self, t: int) -> torch.Tensor:
        """
        Get the input embedding for frame t using streaming inference.
        
        This function simulates real-time microphone input by:
        1. Extracting a window of audio frames [max(0, t-window_size+1) : t+1]
        2. Passing the window through the perception encoder
        3. Returning the embedding of the last frame (frame t)
        
        Args:
            t: Frame index (0-indexed)
        
        Returns:
            input_embed: Embedding for frame t [B, H] where H is hidden size
        
        Example:
            If window_size=70 and t=100:
            - Extract audio frames [31:101] (70 frames)
            - Encode the window
            - Return embedding of frame 100 (last frame of the window)
        
        Note:
            - For t < window_size, we pad with zeros at the beginning
            - This mimics real-world streaming where we have limited history
        """
        if not hasattr(self, 'streaming_audio'):
            raise RuntimeError("Must call init_streaming_inference() before get_input_embeds()")
        
        if t >= self.streaming_total_frames:
            raise ValueError(
                f"Frame index t={t} exceeds total frames {self.streaming_total_frames}"
            )
        
        B = self.streaming_audio.shape[0]
        
        # Calculate the window range [start_frame, end_frame]
        # We want a window of size window_size ending at frame t
        end_frame = t + 1  # Inclusive end
        start_frame = max(0, end_frame - self.streaming_window_size)
        window_frames = end_frame - start_frame
        
        # Extract audio samples for this window
        start_sample = start_frame * self.streaming_samples_per_frame
        end_sample = end_frame * self.streaming_samples_per_frame
        audio_window = self.streaming_audio[:, start_sample:end_sample]
        
        # If we're at the beginning and don't have full window_size frames, pad with zeros
        if window_frames < self.streaming_window_size:
            pad_frames = self.streaming_window_size - window_frames
            pad_samples = pad_frames * self.streaming_samples_per_frame
            audio_window = torch.nn.functional.pad(
                audio_window, (pad_samples, 0), mode='constant', value=0
            )
        
        # Pass through perception encoder
        audio_window_lens = torch.tensor([audio_window.shape[1]], device=audio_window.device)
        source_encoded, lengths, asr_emb = self.perception(
            input_signal=audio_window,
            input_signal_length=audio_window_lens,
            return_encoder_emb=True
        )
        # source_encoded shape: [B, T_frames, H]
        
        # Return the embedding of the last frame (frame t)
        # The last frame in the encoder output corresponds to frame t
        input_embed = source_encoded[:, -1, :]  # [B, H]
    ...
    def offline_inference(
        ...
        ans = {
            "text": tokens_to_str(gen_text, lengths, tokenizer=self.tokenizer, pad_id=self.text_pad_id,
                                  eval_text_turn_taking=self.cfg.get("eval_text_turn_taking", True), sil_id=sil_id),
