...
class DuplexS2SSpeechDecoderModel(LightningModule, HFHubMixin):
    ...
    def prepare_inputs(self, batch: dict):
        ...
        # NOTE: asr_emb is not yet updated to work with the prompt tokens
    ...
    def prepare_t2t_inputs(self, batch: dict):
        ...
        if self._use_tp:
            ...
            if (remainder := (target_tokens.shape[1] - 1) % tp_world_size) != 0:
                ...
                source_tokens = source_tokens[:, :-remainder]
                asr_emb = asr_emb[:, :-remainder]
        ...
        input_embeds = self.embed_tokens(text_inputs)
        source_encoded = self.embed_tokens(source_tokens)  # (B, T-1, H)
        ...
        return {
            "input_embeds": input_embeds,
            "input_lens": batch["source_token_lens"]-1,
            "output_lens": batch["target_token_lens"]-1,
            "text_labels": text_labels,
            "seq_mask": seq_mask,
            "loss_scale": loss_scale,
        }
    ...
    def training_step(self, batch: dict, batch_idx: int):
        ...
        if batch["audio_data"] is not None:
            ...
            if self.cfg.get("add_duplex_text_loss", False):
                inputs = self.prepare_t2t_inputs(batch["audio_data"])
                forward_outputs = self(
                    inputs["input_embeds"],
                    seq_mask=inputs["seq_mask"],
                    target_text_tokens=inputs["text_labels"],
        ...
        res["loss"] = (1. - self.cfg.text_to_text_loss_weight) * res.get("audio_loss", 0.0) + \
            ...
            self.cfg.get("duplex_text_loss_weight", 0.0) * res.get("duplex_text_loss", 0.0)
    ...
    def offline_inference(
        ...
        # If we have prompts, we process all prompt tokens first (prefill phase)
